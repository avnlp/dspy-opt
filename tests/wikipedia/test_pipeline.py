"""Tests for the Wikipedia pipeline."""
...
class TestPipeline:
    """Wikipedia pipeline coverage."""

    def test_rag_pipeline_success_path(self, mocker: pytest_mock.MockFixture) -> None:
        """Validate the happy path through the Wikipedia RAG pipeline."""
        module = importlib.import_module("dspy_opt.wikipedia.wikipedia_rag_module")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("dspy_opt.wikipedia.wikipedia_rag_mipro", "WikipediaRAG", {}),
            ("dspy_opt.wikipedia.wikipedia_rag_copro", "WikipediaRAG", {}),
            ("dspy_opt.wikipedia.wikipedia_rag_simba", "WikipediaRAG", {}),
            (
                "dspy_opt.wikipedia.wikipedia_rag_bootstrap_few_shot",
                "WikipediaRAG",
                {
                    "optimizer": {
            ...
            ("dspy_opt.wikipedia.wikipedia_rag_gepa", "WikipediaRAG", {}),
            (
                "dspy_opt.wikipedia.wikipedia_rag_evaluation",
                "WikipediaRAG",
                {"dataset": {"name": "name", "split": "test", "test_size": 0.1}},
            ),
        ],
    )
    ...
    def test_indexing_script_runs(self, mocker: pytest_mock.MockFixture) -> None:
        ...
        runpy.run_module("dspy_opt.wikipedia.wikipedia_indexing", run_name="__main__")
